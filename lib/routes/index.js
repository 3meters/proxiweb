...
var linkTypeMap = utils.getLinkTypeMap()
var clWhitelist = utils.getClWhitelist()
...
exports.addRoutes = function(app) {
  ...
  app.route('/:clName/:_id/:linkName/:linkedClName*')
    .all(setLinkedOptions)

  app.route('/:clName/:_id/:linkName/:linkedClName/create')
...
function setLinkedOptions(req, res, next) {
  ...
  if (!linkTypeMap[linkName]) return utils.notFound(req, res, next)

  // Build the link query
  var linkQuery = {type: linkTypeMap[linkName].type}
  linkQuery[linkTypeMap[linkName].direction] = linkedClName
  req.sreq.query({linked: linkQuery})

  req.locals.linkName = linkName
  req.locals.linkedClName = linkedClName
  req.locals.linkedSchema = schemas[linkedClName]
...
function getLinked(req, res, next) {
  req.locals.view = 'list'
  req.locals.title = req.locals.schema.name + ' ' +
      req.locals.linkName + ' ' + req.locals.linkedClName
  req.sreq.path('/data/' + req.locals.clName + '/' + req.params._id)
  utils.renderData(req, res, next)
}
...
function showCreateLinked(req, res, next) {
  ...
  // Show details view in create mode
  log(req.locals)
  req.locals.title = 'Create ' + req.locals.linkedSchema.name
  req.locals.mode = 'create'
  req.locals.clName = req.locals.linkedClName
  res.render('details', req.locals)
}
...
function post(req, res, next) {
  ...
  if (req.linkClName) {
    req.sreq.path('/data/' + req.locals.linkedClName)
    var link = {type: linkTypeMap[linkName].type}
    link[linkTypeMap[linkName].direction] =
