...
function start(req, res, next) {
  ...
  if (user) res.redirect('/users/' + user._id + '/watches/patches')
...
function post(req, res, next) {
  ...
  // Need a less tricky way to check insert vs update.
  ...
  if (req.locals.link) {
    ...
    // Post new doc to the linkCl, not the cl
    req.sreq.pathAdd(req.locals.linkCl)

    // The rub: on create doc, create a link from the to-be-created doc
    // to its linked doc in the inverse direction of the link specified
    // in the path.
    //
    var sLink = {type: links[link].type}
    if (links[link].direction === 'to') {
      sLink._from = req.params._id
    }
    else sLink._to = req.params._id

    // Add the link to the insert body if it is not the same as the create link
    if (!_.isEqual(sLink, createLink)) data.links.push(sLink)
  }
